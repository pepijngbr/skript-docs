<template>
  <aside class="absolute hidden xl:block top-4 left-4 w-64">
    ...
    <ul>
      ...
      >
        <p
          class="text-black dark:text-white transition-all duration-100 hover:text-skript hover:font-bold"
        ...
        <ul v-show="openCategories[category]">
          <li
            v-for="element in categories[category]"
            :key="element.name"
            class="ml-3 text-black dark:text-white transition-all duration-100 hover:text-skript hover:font-bold cursor-pointer"
          >
            <a
              :href="'#' + element.prefix + '-' + element.name.replace(/\s+/g, '-').toLowerCase()"
              >{{ element.name }}</a
            >
          </li>
        </ul>
